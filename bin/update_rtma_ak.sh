...
function compute_solar {
   f=`echo ${1}|cut -d '/' -f 10-` #get last two tokens, containing dir and file name
...
for d in `curl -s --list-only ${RTMA_FTP}/|grep 'akrtma\.'|sort` #a list of directories from ftp
do 
...
do 
   ...
   LOCAL_FILES=(`find ${FILE_DIR}/grb -name '*'${PATTERNS[counter]}'*' |cut -d '/' -f 10-|sort`)
   if [ ${d} = 'pcp' ] ; then
   ...
   find ${FILE_DIR}/grb -empty -type d -name 'akrtma.*' -delete
   (( counter++ ))
done
...
do 
   ...
   for f in `find ${FILE_DIR}/tif/${var} -path '*akrtma*.grb2' -type f|rev|cut -d '/' -f 1,2|rev`
   do 
	   if [ ! -f  ${FILE_DIR}/grb/$f ] ; then 
   ...
   #Remove empty directories:
   find ${FILE_DIR}/tif/${var} -empty -type d -name 'akrtma.*' -delete
   (( counter++ ))
done
