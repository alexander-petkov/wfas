...
void Usage(const char *pszError)
{
    printf("solar_grid [--perc-cloud-cover percent] [--cloud-file cloud_raster]\n"
	   "	   [--aspect-file aspect_raster] [--slope-file slope_raster]\n"
	   "	   [--shade-file shade_raster]\n"
...
int main(int argc, char *argv[])
{
    ...
    const char *pszAspectFile = NULL;
    const char *pszSlopeFile = NULL;
    const char *pszShadeFile = NULL;
    ...
    while(i < argc)
    {
        ...
        else if(EQUAL(argv[i], "--cloud-file"))
        {
            pszCloudFile = argv[++i];
        }
        else if(EQUAL(argv[i], "--aspect-file"))
        {
            pszAspectFile = argv[++i];
        }
        else if(EQUAL(argv[i], "--slope-file"))
        {
            pszSlopeFile = argv[++i];
        }
        else if(EQUAL(argv[i], "--shade-file"))
        {
            pszShadeFile = argv[++i];
        }
    ...
    Aspect asp;
    if (pszAspectFile != NULL)
    {
    	asp.GDALReadGrid(pszAspectFile, 1);
    	if(dfCellSize > 0)
        	asp.resample_Grid_in_place(dfCellSize, AsciiGrid<double>::order1);
    }
    else
    {
	asp = Aspect(&elev,nNumThreads);
    }
    Slope slp;
    if (pszSlopeFile != NULL)
    {
    	slp.GDALReadGrid(pszSlopeFile, 1);
    	if(dfCellSize > 0)
        	slp.resample_Grid_in_place(dfCellSize, AsciiGrid<double>::order1);
    }
    else
    {
	slp = Slope(&elev,nNumThreads);
    }
    Shade shd;
    if (pszShadeFile != NULL)
    {
    	shd.GDALReadGrid(pszShadeFile, 1);
    	if(dfCellSize > 0)
        	shd.resample_Grid_in_place(dfCellSize, AsciiGrid<short int>::order1);
    }
    else
    {
	shd = Shade(&elev, solar.get_theta(), solar.get_phi(), nNumThreads);
    }
    ...
    /*
    asp.write_Grid("aspect.asc", 0);
    slp.write_Grid("slope.asc", 0);
    shd.write_Grid("shade.asc", 0);
    */
#ifdef _OPENMP
