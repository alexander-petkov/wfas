...
public class LandscapeExportWPS implements GeoServerProcess {
	...
	private static final Logger LOGGER =
            Logger.getLogger(LandscapeExportWPS.class.toString());
	private String wkt;
	...
	public GridCoverage2D execute(@DescribeParameter(name = "Longitude", 
									description = "Center Longitude for Landscape file")
									Double lon,
									@DescribeParameter(name = "Latitude", 
									description = "Center latitude for Landscape file") 
									Double lat,
		...
		try {
			...
			transform =
					CRS.findMathTransform(CRS.decode("EPSG:" + input.getSRID()), crs);
			transPoint = (Point) JTS.transform(input, transform);
		}catch(Exception e) {
			if (LOGGER.isLoggable(Level.FINE)) {
				LOGGER.fine(
						"Unable to parse Albers projection from input coords: " + e.toString());
			}//end if
		}//end catch

		ReferencedEnvelope envelope =
                new ReferencedEnvelope(-48280.3,48280.3,-48280.3,48280.3,crs);
		...
		/*
		 * Convert our custom Albers-generated envelope to 
		 * coverage CRS envelope:
		 */
        ...
        GeometryCollection roi = geometryFactory.createGeometryCollection(new Geometry[] {JTS.toGeometry(coverageEnv)});
		...
		GridCoverage2D result = handleReprojection((GridCoverage2D) PROCESSOR.doOperation(param),
													crs, 
	 ...
	 * @param lon
	 * @param lat
	 * @return custom Albers WKT centered at input coords, 
	 * and with standard parallels 60 miles apart from each other.
	 */
...
//		double fraction = 60/miles;
		...
		/*
		 * One degree latitude is approx 69 miles. 
		 * We are looking for 60 miles extent,
		 * so set the standard parallels 60 miles apart
		 * (60/69)/2=0.434 degrees latitude in each direction. 
		 */
    ...
    /**
     * This method is responsible for cropping the provided {@link GridCoverage} using the provided
     * subset envelope.
     *
