...
SUBREGION='subregion=&leftlon=220&rightlon=300&toplat=53&bottomlat=22'
...
function derive_wdir {
   ...
   do 
	${CDO_DIR}/cdo -O -b 10 expr,'10wdir=10u' -mulc,57.3 -atan2 -mulc,-1 ${GFS_DIR}/UGRD/gfs.${HOUR}.pgrb2.${RES}.f${h}.grb2 -mulc,-1 ${GFS_DIR}/VGRD/gfs.${HOUR}.pgrb2.${RES}.f${h}.grb2 ${GFS_DIR}/WDIR/gfs.${HOUR}.pgrb2.${RES}.f${h}.grb2
   done
}
function derive_wspd {
   ...
   do 
      ...
      ${CDO_DIR}/cdo -O expr,'10si=(sqrt(10u*10u+10v*10v))' ${GFS_DIR}/out.grb2 ${GFS_DIR}/WSPD/gfs.${HOUR}.pgrb2.${RES}.f${h}.grb2
   done
...
#1. If dataset is not derived, get data
   if [ ${DERIVED[$counter]} = 0 ]; then 
      for h in `seq -w 003 1 384`
      do 
         #wget  "${NOMADS_URL}?file=gfs.${HOUR}.pgrb2.${RES}.f${h}&lev_${LEVEL[$counter]}=on&var_${d}=on&${SUBREGION}&dir=%2F${FORECAST}%2F00" \
		#-O ${FILE_DIR}/gfs.${HOUR}.pgrb2.${RES}.f${h}.grb2;
